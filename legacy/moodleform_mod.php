...
class moodleform_mod extends moodleform {
    ...
    function definition_after_data() {
        ...
        if ($id = $mform->getElementValue('update')) {
            ...
            if (! empty($this->_features->gradecat)) {
                $gradecat = false;
                ...
                if ($items = grade_item::fetch_all(array('itemtype'=>'mod', 'itemmodule'=>$modulename,
                                                   'iteminstance'=>$instance, 'courseid'=>$courseid))) {
    ...
    function validation($data, $files) {
        ...
        $grade_item = grade_item::fetch(array('itemtype'=>'mod', 'itemmodule'=>$data['modulename'],
                     'iteminstance'=>$data['instance'], 'itemnumber'=>0, 'courseid'=>$courseid));
        ...
        if ($mform->elementExists('cmidnumber')) {
            // verify the idnumber
            if (!grade_verify_idnumber($data['cmidnumber'], $courseid, $grade_item, $cm)) {
    ...
    function standard_coursemodule_elements($features=null){
        ...
        if ($CFG->majorrelease<=1.4) {
            // do nothing - no visible standard elements on Moodle 1.4 and earlier
        } else {
            ...
            if ($CFG->majorrelease<=1.6) {
                $str = get_string('visibletostudents', '', moodle_strtolower($COURSE->students));
            } else {
                $str = get_string('visible');
            }
            $mform->addElement('modvisible', 'visible', $str);
            ...
            if ($this->_features->gradecat) {
                $categories = grade_get_categories_menu($courseid, $outcomesused);
                $mform->addElement('select', 'gradecat', get_string('gradecategory', 'grades'), $categories);
            }
        } // end if ($CFG->majorrelease<=1.4)
