...
// fix container table width on mod.html page in Moodle <= 1.4
var tables = document.getElementsByTagName('table');
if (tables) {
    var i_max = tables.length;
    for (var i=0; i<i_max; i++) {
        if (tables[i].className=='generalbox' && tables[i].id=='') {
            tables[i].id = 'quizport';
            var obj = document.getElementsByTagName('body');
            if (obj && obj.length && obj[0].id=='') {
                obj[0].id = 'course-mod';
            }
            obj = null;
            break;
...
function setfocus() { if (document.form) document.form.name.focus() }