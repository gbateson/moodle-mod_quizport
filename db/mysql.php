...
function quizport_upgrade($oldversion, $module=null, $stopversion=0) {
    ...
    if ($result && $oldversion < $newversion) {
        ...
        foreach ($fields as $field => $afterfield) {
            $rs = $db->Execute("SHOW COLUMNS FROM $table LIKE '$field'");
            if (empty($rs) || $rs->RecordCount()==0) {
                // field does not exist, so create it
                table_column($table, '', $field, 'integer', 10, '', 0, 'not null', $afterfield);
            }
        }
    }
